...
void find_number_book();    //按书号查询
void find_publish_book();   //按出版社查询 

void HideCursor()           //隐藏光标(用来美化，百度现学的)
{
 CONSOLE_CURSOR_INFO cursor_info = {1, 0}; 
 SetConsoleCursorInfo(GetStdHandle(STD_OUTPUT_HANDLE), &cursor_info);
}

void toxy(int x, int y)      //将光标移动到X,Y坐标处（用来美化）
{
COORD pos = { x , y };
	...
	do
	{
		...
		while(p!=NULL)
		{
			if(p!=NULL&&strcmp(p->name,_name)==0)
			{
				toxy(20,i);
				j++;
				printf("%d:%s%14s%14s   %14s   %18s         %.2f%12d\n",j,p->number,p->name,p->author,p->publish,p->time,p->price,p->num);
				p->x=j;    //给符合查询标准的结点标号 
				i++;
			}
			p=p->next;
...
void print_book()   //图书总览 
{
	...
	{
		toxy(20,i);
		printf("%s%14s%14s   %14s   %18s         %.2f%12d\n",p->number,p->name,p->author,p->publish,p->time,p->price,p->num);
		i++;
		sum+=p->num;//计算图书总量 
		p=p->next;
	}
